	...
	public class RegisterModel : PageModel
	{
		...
		public class InputModel
		{
			...
			[RegularExpression("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{6,}$",
				ErrorMessage = "Your password must contain at least 6 characters, a capital letter, a lowercase letter, a number, and a special character.")]
		...
		public async Task<IActionResult> OnPostAsync(string returnUrl = null)
		{
			...
			if (ModelState.IsValid)
			{
				...
				{
					ModelState.AddModelError("Input.Username", "This username is already registered!");
					return Page();
				}

				var emailExists = _userManager.FindByEmailAsync(Input.Email).GetAwaiter().GetResult();
				if (emailExists != null)
				{
